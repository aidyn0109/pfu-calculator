...
# осталось справочным (нижний уровень REVENUE_CAPS / PFU_CAPS).
S_MRP_MIN = 800_000
...
> 800 000 МРП считаются по нижнему уровню капов (см. шаги 3 и 6).
...
```
S_mrp ≤ 1 600 000             → Revenue_cap = 200%
1 600 000 < S_mrp ≤ 3 200 000 → Revenue_cap = 500%
S_mrp > 3 200 000             → Revenue_cap = 700%
```

> Нижний уровень действует и для сумм меньше 800 000 МРП (порог снят).
...
годы, по которым выполнялся расчёт. Требует входа.
...
| `S_mrp < 800 000` | Расчёт выполняется (порог снят); капы — нижнего уровня |
...
test_caps_below_800k_use_lowest_tier()  # ниже 800 000 МРП → капы 200% / 365%
...
*Последнее обновление: 2026-08 (добавлен 2025 год и выбор годов расчёта на
калькуляторе; авто-миграция колонок; фикс prepared statements asyncpg через
pgbouncer). Ранее: авторизация с ролями, загрузка Excel через браузер,
поддержка PostgreSQL/asyncpg, деплой на Render.*  
