...
- **Калькулятор MDE** — те же формулы, но пороги, шаги, коэффициенты и
  ...
  вводит пользователь. Второго алгоритма нет — отличаются только числа.
...
знаменатели показателей и капы итогового ПФУ — они всегда из `config.py`.
...
6. Список параметров, которые вкладка MDE отдаёт пользователю (раздел 7.4):
   формулу дохода, знаменатели показателей и капы итогового ПФУ пользователь
   задавать не должен
...
*Последнее обновление: 2026-09 (вкладка «Калькулятор MDE» с ручным вводом
параметров формул; показатель ФОТ считается от Revenue_sum, а не от суммы S —
изменение затрагивает ОБЕ вкладки; колонки kind/params в calculations).
