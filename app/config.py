...
# Каждый элемент: (S_mrp_min, S_mrp_max, cap_percent). Верхняя граница
# включительна; диапазоны проверяются по порядку — первый подходящий выигрывает.
# Нижняя граница первого диапазона справочная: суммы меньше 800 000 МРП тоже
# считаются и получают кап этого нижнего уровня.
